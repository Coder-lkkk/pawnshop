...
<script type="text/javascript">
    function getStoneList() {
        $.ajax({
            type:"get",
            url:"<%=basePath%>continue/getStoneList",
            data:{
            	"uid":13
            },
            success:function(data) {
            	    ...
            	    for(i=1;i<=list.length;i++){
	            	    $("#id-"+i).text("编号："+list[i-1].sid);             //为相应的id传入相应的值
	            	    $("#name-"+i).text(list[i-1].sname);
	            	    $("#style-"+i).text(list[i-1].sstyle);
        				$("#type-"+i).text(list[i-1].stype);
       					$("#id_"+i).text(list[i-1].sid);
       					$("#pawntime-"+i).text(list[i-1].spawntime);
       					$("#btn-"+i).attr('onclick',"continue_("+list[i-1].sid+")");           //为相应的按钮的onclick传入相应的参数
            	    }
            }
        });
    }
...
function continue_(i){
    	...
    	var id = i;
    	window.location.href = "<%=basePath%>continue/stoneContinue?sid="+i;
    }
</script>
<script>
                ...
                html =`<li data- class="dy-video-item dy-video-meta-left">
                        ...
                        <div class="dy-video-meta-dy">
                            ...
                            <ul class="dy-video-meta-list">
                                ...
                                <li class="dy-video-types">
                                    <span class="dy-video-tip">种类:</span>
                                    <span id="style-`+ i +`" class="dy-video-meta-filter"></span>
                                </li>
                    ...
                    <div class="dy-video-poster">
                        <a class="dy-video-link">
                            <img class="dy-video-img" src="<%=basePath%>images/s.jpg" alt="编号">
                        </a>
                    </div>
                    <!--详情开头-->
