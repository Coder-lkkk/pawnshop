...
<div class="xu_box_header">
    <div class="xu_box_header_content">
        ...
        <a class="back" href="Continued.html">返回续当页面</a>
    </div>
</div>
<!--图文详情-->
<div class="main">
...
<script type="text/javascript">
    function getWatchList() {
        $.ajax({
            type:"get",
            url:"<%=basePath%>continue/getWatchList",
            data:{
            	"uid":13
            },
            success:function(data) {
            	    ...
            	    for(i=1;i<=list.length;i++){
	            	    $("#id-"+i).text("编号："+list[i-1].wid);             //为相应的id传入相应的值
	            	    $("#brand-"+i).text(list[i-1].wbrand);
	            	    $("#style-"+i).text(list[i-1].wstyle);
        				$("#material-"+i).text(list[i-1].wmaterial);
       					$("#id_"+i).text(list[i-1].wid);
       					$("#pawntime-"+i).text(list[i-1].wpawntime);
       					$("#btn-"+i).attr('onclick',"continue_("+list[i-1].wid+")");           //为相应的按钮的onclick传入相应的参数
            	    }
            }
        });
    }
...
function continue_(i){
    	...
    	var id = i;
    	window.location.href = "<%=basePath%>continue/watchContinue?wid="+i;
    }
</script>
<script>
