...
<head>
    <meta charset="UTF-8">
    <title>在线续当-奢侈品箱包</title>
    <link rel="stylesheet" href="<%=basePath%>css/bootstrap.min.css">
    <link rel="stylesheet" href="<%=basePath%>css/j_con tinued.css">
</head>
<body>
<!--首页-->
<div class="xu_box_header">
    <div class="xu_box_header_content">
        ...
        <a class="back" href="<%=basePath%>redemption/toRedemption">返回续当页面</a>
    </div>
</div>
<!--图文详情-->
<div class="main">
...
<script type="text/javascript">
    function getBagList() {
        $.ajax({
            type:"get",
            url:"<%=basePath%>continue/getBagList",
            data:{
            	"uid":13
            },
            success:function(data) {
            	    ...
            	    for(i=1;i<=list.length;i++){
	            	    $("#id-"+i).text("编号："+list[i-1].bid);             //为相应的id传入相应的值
	            	    $("#brand-"+i).text(list[i-1].bbrand);
	            	    $("#length-"+i).text(list[i-1].blength);
       					$("#id_"+i).text(list[i-1].bid);
       					$("#pawntime-"+i).text(list[i-1].bpawntime);
       					$("#btn-"+i).attr('onclick',"continue_("+list[i-1].bid+")");           //为相应的按钮的onclick传入相应的参数
            	    }
            }
        });
    }
...
function continue_(i){
    	alert("你的续当申请已提交，请留意消息");
    	var id = i;
    	window.location.href = "<%=basePath%>continue/bagContinue?bid="+i;
    }
</script>
<script>
        ...
        for(var i=1;i<=len;i++){
                          ...
                          <div class="modal-dialog modal-lg  modal-dialog-centered">
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    <button id="btn-`+ i +`" class="btn btn-primary btn-danger">续当</button>
                                </div>
                            </div>
                        </div>
                    </div> `;
            if(i%3===0){               //如果位于每一行的最右边 则拥有不同的css样式
                html =`<li data- class="dy-video-item dy-video-meta-left">
                        ...
                        <div class="dy-video-meta-dy">
                            ...
                            <ul class="dy-video-meta-list">
                                ...
                                <li class="dy-video-types">
                                    <span class="dy-video-tip">包体长度:</span>
                                    <span id="length-`+ i +`" class="dy-video-meta-filter"></span>
                                </li>
                            </ul>
                    ...
                    <div class="dy-video-poster">
                        <a class="dy-video-link">
                            <img class="dy-video-img" src="<%=basePath%>images/bags.jpg" alt="编号">
                        </a>
                    </div>
                    <!--详情开头-->
                    <div class="dy-video-primary">
                        <h4 class="dy-video-title">
                            <a href="#modal`+ i +`" data-toggle="modal">在线续当</a>
                        </h4>
                    </div>
                </li>`;
                ...
                html =`<li data- class="dy-video-item dy-video-meta-right">
                    ...
                    <div class="dy-video-primary">
                        <h4 class="dy-video-title">
                            <a id="detail" href="#modal`+ i +`" data-toggle="modal">在线续当</a>
                        </h4>
                    </div>
                </li>`;          
            }
