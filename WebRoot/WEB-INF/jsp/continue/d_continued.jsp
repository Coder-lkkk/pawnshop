...
<script type="text/javascript">
    function getDiamondList() {
        $.ajax({
            type:"get",
            url:"<%=basePath%>continue/getDiamondList",
            data:{
            	"uid":13
            },
            success:function(data) {
            	    ...
            	    for(i=1;i<=list.length;i++){
	            	    $("#id-"+i).text("编号："+list[i-1].did);             //为相应的id传入相应的值
	            	    $("#brand-"+i).text(list[i-1].dbrand);
	            	    $("#shape-"+i).text(list[i-1].dshape);
        				$("#carat-"+i).text(list[i-1].dcarat);
       					$("#id_"+i).text(list[i-1].did);
       					$("#pawntime-"+i).text(list[i-1].dpawntime);
       					$("#btn-"+i).attr('onclick',"continue_("+list[i-1].did+")");           //为相应的按钮的onclick传入相应的参数
            	    }
            }
        });
    }
...
function continue_(i){
    	...
    	var id = i;
    	window.location.href = "<%=basePath%>continue/diamondContinue?did="+i;
    }
</script>
<script>
                ...
                html =`<li data- class="dy-video-item dy-video-meta-left">
                        ...
                        <div class="dy-video-meta-dy">
                            ...
                            <ul class="dy-video-meta-list">
                                ...
                                <li class="dy-video-types">
                                    <span class="dy-video-tip">形状:</span>
                                    <span id="shape-`+ i +`" class="dy-video-meta-filter"></span>
                                </li>
                                <br>
                                <li class="dy-video-areas">
                                    <span class="dy-video-tip">克拉:</span>
                                    <span id="carat-`+ i +`" class="dy-video-meta-filter"></span>
                                </li>
                            </ul>
                    ...
                    <div class="dy-video-poster">
                        <a class="dy-video-link">
                            <img class="dy-video-img" src="<%=basePath%>images/d.jpg" alt="编号">
                        </a>
                    </div>
                    <!--详情开头-->
